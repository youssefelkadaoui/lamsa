...
const normalizeImageUrl = value => {
  ...
  return (markdownMatch ? markdownMatch[1] : text).replace(/^\[|\]$/g, '').trim();
};
...
function getProductImages(product) {
  if (!product) return [];
  let list = [];
  const storedImages = product.additional_images ?? product.images;
  if (storedImages) {
    try { const parsed = typeof storedImages === 'string' ? JSON.parse(storedImages) : storedImages; if (Array.isArray(parsed)) list = parsed.filter(Boolean); } catch (e) {}
  }
...
async function saveProduct(event) {
  ...
  let image = normalizeImageUrl($('imagePage').value);
  const extraImages = [];
  try {
  ...
  const imagesArray = [image, ...extraUrls, ...extraImages].filter(Boolean);
  const payload = { name: $('namePage').value.trim(), category: $('categoryPage').value, description: $('descriptionPage').value.trim(), original_price: originalPrice, new_price: newPrice, image: imagesArray[0] || image, additional_images: imagesArray, stock_status: $('stockStatusPage').value, is_offer: isOffer, offer_discount: isOffer ? $('productOfferDiscountPage').value.trim() : null, offer_expires_at: isOffer ? new Date(offerExpires).toISOString() : null };

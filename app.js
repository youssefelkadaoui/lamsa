...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('productForm').addEventListener('submit', async e => {
    ...
    let image = normalizeImageUrl(document.getElementById('image').value);
    const extraImages = [];
    try {
    ...
    const additionalImages = [image, ...extraUrls, ...extraImages].filter(Boolean);
    ...
    const productPayload = {
      name, category, image: additionalImages[0] || image, additional_images: additionalImages, description, original_price: originalPrice, new_price: newPrice,
